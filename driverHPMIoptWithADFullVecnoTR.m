...
if optf
    ...
    stateconstraint  = optimconstr(    [Ntime*lqp,Nspecies,]);
    ...
    aifplaceholder =     rand(Ntime*lqp,2);
    currentTR = 2;
    nsubstep = 5;
    ...
    subdiagpyr = repmat(  cos(FaList(1,:)),1,lqp);
    subdiaglac = repmat(  cos(FaList(2,:)),1,lqp);
    pyruvatematrix = diag(ones(Ntime*lqp,1)) + diag( - subdiagpyr(1:(lqp*Ntime-1)),-1);
    lactatematrix  = diag(ones(Ntime*lqp,1)) + diag( - subdiaglac(1:(lqp*Ntime-1)),-1);
    stateconstraint(:,1)  = pyruvatematrix * statepyruvate(:) == aifterm(:,1) ;
    stateconstraint(:,2)  = lactatematrix * statelactate(:)   == aifterm(:,2) ; 

    disp('build objective function')
    %statematrix = optimexpr([lqp,lqp]);
    diffsummone = repmat(sum(statepyruvate,1)',1,lqp) - repmat(sum(statepyruvate,1) ,lqp,1);
    diffsummtwo = repmat(sum(statelactate ,1)',1,lqp) - repmat(sum(statelactate ,1) ,lqp,1);
    Hz = 0;
    for jjj=1:lqp2
      znu=xn2{1}(jjj) ;
    ...
    x0.statepyruvate  = zeros(Ntime,lqp);
    x0.statelactate   = zeros(Ntime,lqp);
